...
class SupplyRiskModel:
    def __init__(self, business_constants):
        self.D = business_constants.get('D', 1667)
        self.C0 = business_constants.get('C0', 1667)
        self.TTS = business_constants.get('TTS', 30)
        self.W_raw = business_constants.get('W_raw', 0.75)
        self.Cost_limit_ratio = business_constants.get('Cost_limit_ratio', 1.15)
        self.ST = 100
        self.Y_target = 0.85
        self.k = 0.005
        self.alpha = 0.5
    ...
    def model_2_politics(self, S_ban=0, T_policy=0, R_tariff=0, T_delay=0):
        ...
        if S_ban == 1:
            ...
            if t_dead < self.time_horizon: curve[t_dead:] = 0
    ...
    def model_3_tech(self, t_LTB=360, Y_new=0.9, R_pen=0.4):
        ...
        for i in range(len(self.t)):
            if self.t[i] >= t_LTB: curve[i] = 0
        return curve

    def model_4_competition(self, U_current=0.8, C_new=0, R_growth=0.1, I_shift=0, T_cross=0.2):
        C_total = self.C0 * 30 / U_current if U_current > 0 else 0
        buffer = (1 - U_current) * C_total + C_new
        curve = []
        for day in self.t:
            month = day / 30
            delta_D_cross = T_cross * C_total * ((1 + R_growth)**month - 1)
            s_cross = 1 - max(0, (delta_D_cross - buffer) / (self.C0 * 30)) * (1 + self.alpha * I_shift) if self.C0 > 0 else 0
    ...
    def model_8_commercial(self, R_raw=0.05, R_price=0.02, GM=0.2, INV_raw=30, I_stop=0):
        if I_stop == 1: return np.zeros(self.time_horizon), "直接断供"
    ...
    def plot_results_for_vercel(self, results, material_info):
        fig, axes = plt.subplots(4, 2, figsize=(15, 20))
        axes = axes.flatten()
        fig.suptitle(f"物料: {material_info.get('name', '')} - 供应链风险分析", fontsize=20, y=1.02)

        for i, (name, curve) in enumerate(results.items()):
            axes[i].plot(self.t, curve, label='预测产能', color='red', linewidth=2)
            axes[i].axhline(y=self.D, color='blue', linestyle='--', label=f'需求线 ({self.D})')
            ...
            if curve is not None and hasattr(curve, '__iter__'):
                shortage_days = np.where(curve < self.D)[0]
                if len(shortage_days) > 0:
                    first_shortage_day = shortage_days[0]
                    axes[i].scatter(first_shortage_day, curve[first_shortage_day], color='black', zorder=5, s=80)
                    max_val = curve.max() if curve.max() > 0 else 10
                    axes[i].annotate(f'断点: 第{first_shortage_day}天',
                                     xy=(first_shortage_day, curve[first_shortage_day]),
                                     xytext=(first_shortage_day + 10, curve[first_shortage_day] + (max_val * 0.1)),
    ...
    def generate_perception_table(self, material_info, params):
        ...
        data = [
            ...
            },
            {
                ...
                "信息来源与原文链接": f"【原文】: 商务部公告：自2026年起加强对特定工业原材料的出口最终用途审查，预计审批周期增加。\n【链接】: http://www.mofcom.gov.cn/",
            ...
            },
            {
                ...
                "信息来源与原文链接": f"【原文】: 全球货运指数显示，当前主要航线交期维持在正常水平，港口拥堵指数处于‘低风险’。\n【链接】: https://www.marinetraffic.com/",
            ...
            },
            {
                ...
                "信息来源与原文链接": f"【原文】: 关键原材料现货价格近期波动剧烈，下游企业面临成本压力加剧，供应商面临毛利挑战。\n【链接】: https://www.investing.com/commodities/",
    ...
    def generate_recommendations(self, material_info, params, results):
        ...
        if ms > params['8_GM'] and params['8_GM'] > 0:
